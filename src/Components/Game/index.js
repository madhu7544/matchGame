...
class Game extends Component {
  ...
  generateRandomImg = () => {
    const random = Math.ceil(Math.random() * imagesList.length)
  ...
  renderGame = () => {
    const {activeTabId, displayImg} = this.state
    const filteredImage = this.filterImagesList(activeTabId)

    return (
      <div className="game-section">
        <div className="match-image-container">
          <img src={displayImg.imageUrl} alt="match" className="match-image" />
        </div>
  ...
  render() {
    const {showResults, score, isTimer} = this.state
    return (
      <div className="bg-container">
        <Navbar score={score} isTimer={isTimer} />
        <div className="game-container">
          {showResults ? (
            this.renderGame()
          ) : (
            <EndGame score={score} onReset={this.onReset} />
          )}
        </div>
      </div>
    )
